...
/* ===== 顾问初判文案（根据选择数量）——植入数据背书 ===== */
const INSIGHTS = {
  low:  '问题比较集中，调整起来不复杂。根据平台 60 万+ 商家的经营数据，你这类情况只要找对方向，往往一步就能见效。我们整理了同行业的真实案例供你参考。',
  mid:  '这几个问题经常一起出现，背后通常是缺少系统规划。好消息是，从我们服务的 60 万+ 商家数据来看，这类情况在你的行业里已经有成熟的解决路径和可复制的案例。',
  high: '问题覆盖面比较广，不是单一环节的事。我们基于 60 万+ 商家经营数据发现，这类情况需要先做一次整体梳理，理清优先级再逐步推进——已经有不少同行走通了这条路。'
};
